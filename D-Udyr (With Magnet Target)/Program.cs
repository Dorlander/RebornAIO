...
using D_Udyr.Helpers;
using SharpDX;
using SharpDX.Multimedia;
using Color = System.Drawing.Color;
using Orbwalking = D_Udyr.Helpers.Orbwalking;
using Environment = D_Udyr.Helpers.Environment;
...
namespace D_Udyr
{
    internal class Program
    {
        ...
        private static void Usepotion()
        {
                     ...
                     )))
            {
                if (iusepotionhp && iusehppotion &&
                     !(ObjectManager.Player.HasBuffIn("RegenerationPotion", 1f, true) ||
                       ObjectManager.Player.HasBuffIn("ItemCrystalFlask", 1f, true) ||
                       ObjectManager.Player.HasBuffIn("ItemMiniRegenPotion", 1f, true)))
                {
                ...
                if (iusepotionmp && iusemppotion &&
                    !(ObjectManager.Player.HasBuffIn("FlaskOfCrystalWater", 1f, true) ||
                      ObjectManager.Player.HasBuffIn("ItemCrystalFlask", 1f, true) ||
                      ObjectManager.Player.HasBuffIn("ItemMiniRegenPotion", 1f, true)))
                {
                    if (Items.HasItem(2041) && Items.CanUseItem(2041))
        ...
        private static void Combo()
        {
            ...
            {
                Smiteontarget();
                if (_e.IsReady() && !target.HasBuffIn("udyrbearstuncheck", 0.5f, true))
                {
                    _e.Cast();
                    return;
                }
                ...
                if (_r.IsReady() && target.HasBuffIn("udyrbearstuncheck", 0.5f, true))
                {
                    _r.Cast();
                    return;
                }

                if (_q.IsReady() && target.HasBuffIn("udyrbearstuncheck", 0.5f, true))
                {
                    _q.Cast();
                    return;
                }

                if (_w.IsReady() && target.HasBuffIn("udyrbearstuncheck", 0.5f, true))
                {
                    _w.Cast();
                    return;
                }
