...
return;
        ...
        {
            CheckAutoWindUp();
            if (CurrentMode == Mode.None || MenuGUI.IsChatOpen || CustomOrbwalkMode || MyHero.IsChannelingImportantSpell() || MyHero.HasBuffIn("katarinarsound",0f, true))
                return;
        ...
        private static void OnDraw(EventArgs args)
        {
                ...
                Menu.Item("orb_Draw_nearKill").GetValue<Circle>().Active)
            {
                ...
                foreach (var minion in minionList.Where(minion => minion.IsValidTarget(GetAutoAttackRange() + 500)))
                {
                    ...
                    if (minion.HasBuffIn("turretshield",0f, true))
                        barWidth = 70;
